...
import type { OptionLeg, StrategyType, Greeks } from '@/types'
...
export type Position = {
  ...
  legs: OptionLeg[]      // with premium = opening premium
  netPremium: number      // CREDIT > 0 / DEBIT < 0
  netGreeks: Greeks
  contracts: number
  openedAt: number       // unix ms
  // mark-to-market (refreshed manually); when null, treat as = opening
  lastMarkPremium?: number
  lastMarkAt?: number
}
...
const positions = ref<Position[]>(load())
...
export function usePositions() {
  function add(p: Omit<Position, 'id' | 'openedAt'>): Position {
    const full: Position = {
      ...p,
      id: uid(),
      openedAt: Date.now()
  ...
  function openCost(p: Position): number {
  ...
  const heldSymbols = computed(() =>
    [...new Set(positions.value.map((p) => p.symbol.toUpperCase()))]
  )
